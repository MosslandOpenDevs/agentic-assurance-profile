...
이 프로필은 Donald Knuth가 이전 시대에 답했던 질문에서 출발합니다. 《TeX: The Program》은 프로그램을 단지 실행되는 것이 아니라 사람에게 설명되어야 하는 것으로 다루었습니다. 설계 이유, 불변조건, 올바름에 대한 논증이 작업 자체의 일부였습니다. AI 코딩 에이전트는 그 규율을 선택 사항으로 만들었던 경제 구조를 뒤집습니다. 구현은 값싸졌지만 설계 이유·불변조건·증거·알려진 한계는 저절로 기록되지 않으며, 코드가 의도를 기록하는 속도보다 빨리 생산될 때 프로젝트가 잃는 것이 바로 그것들입니다.

직접적인 계기는 실무였습니다. Mossland OpenDevs의 첫 파일럿 프로젝트인 Passport는 통상적인 코드 에디터 없이 거의 전적으로 AI 코딩 에이전트로 개발되었습니다. 그 방식으로 일해 보면 공백이 구체적으로 드러납니다. 소유자의 역할은 코드를 작성하는 것에서 주장·불변조건·증거·잔여 위험을 통치하는 것으로 이동하며, 그 통치에는 지속되고 검사 가능한 형식이 필요합니다. 이 프로필이 그 형식입니다.
