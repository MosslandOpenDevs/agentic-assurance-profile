...
| `archived` | 운영·유지보수·기능 개발이 모두 멈춘 참고용 저장소 |

`service`, `trust-critical`, `data-curation`, `agent-runtime`은 `profiles:`에 `core`를 적지 않아도 모든 `core` 의무를 상속합니다. `archived`는 단독으로 선언하고, `paths.system`이 가리키는 system 산출물(기본값 `assurance/SYSTEM.md`)에 네 가지 필수 역사 정보를 기록합니다.
...
1. 운영·유지보수·기능 개발이 모두 중단되었다는 증거를 수집합니다.
2. `[archived]`만 단독 선언하고, `paths.system`이 가리키는 system 산출물에 §6.6의 네 가지 사실을 모두 기록합니다 — 운영·유지보수·기능 개발이 모두 중단된 상태, 역사적 목적, 알려진 중대한 한계, 마지막 지원 revision 또는 release(없다면 없다는 명시적 진술).
