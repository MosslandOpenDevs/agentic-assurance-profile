...
An AI-generated explanation is not evidence by itself. Committed prose inherits this rule when its authorship is agent-assisted (check the introducing commit's co-authorship trailers with `git blame`): cite such text as a description of behavior, never as human intent. Intent authority comes from a human act — a reviewed merge or a recorded review outcome — not from who typed the text.
