...
The two obligations above are fixed, not per-adopter choices. An adoption declaration therefore MUST record `issue_integration.public_security_issues_allowed` as `false` and `issue_integration.closing_requires_artifact_update` as `true`. This holds for a private repository as well as a public one: repository visibility bounds who can read a finding, not whether the profile permits filing an exploitable one as an ordinary Issue.
